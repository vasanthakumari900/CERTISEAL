  ...
  return (
    <div className="min-h-screen bg-navy-950 text-slate-100 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full space-y-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-800 pb-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/10 border border-purple-500/30 text-purple-300 text-xs font-semibold mb-2">
              ...
              Append-Only Database Hash Chain Scanner — CERTX Admin
            </div>
          ...
          <button
            onClick={fetchLedger}
            disabled={loading}
            className="px-4 py-2.5 bg-purple-600 hover:bg-purple-500 text-white font-bold rounded-xl text-xs transition-all flex items-center gap-2 shadow-lg"
          >
        ...
        <div className="bg-navy-900/80 rounded-2xl border border-slate-800 p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded bg-amber-500/20 text-amber-300 font-mono text-[10px] font-bold">
              SIH TAMPER SIMULATOR
            </div>
            <h3 className="text-sm font-bold text-white">Controlled Ledger Block Tamper Simulator</h3>
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <button
              ...
              className="flex-1 sm:flex-initial px-4 py-2 bg-rose-950 hover:bg-rose-900 border border-rose-700 text-rose-200 font-bold rounded-xl text-xs transition-all flex items-center justify-center gap-1.5"
            >
        ...
        {actionMessage && (
          <div className="p-3.5 rounded-xl bg-purple-950/80 border border-purple-700 text-purple-200 font-mono text-xs">
            {actionMessage}
          </div>
        )}
        ...
        {ledgerData && (
          ...
          >
            <div className="flex items-center gap-4">
              <div className={`p-3 rounded-xl ${ledgerData.isIntegrityValid ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}`}>
            ...
            <div className="text-right font-mono text-xs">
              ...
              <p className="text-slate-300">Status: <span className={ledgerData.isIntegrityValid ? 'text-emerald-400 font-bold' : 'text-rose-400 font-bold'}>{ledgerData.isIntegrityValid ? 'PRISTINE' : 'CORRUPTED'}</span></p>
            </div>
          </div>
        )}
                ...
                return (
                  ...
                  >
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b border-slate-800/80 pb-2 mb-2">
                      ...
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${isCorrupted ? 'bg-rose-500/30 text-rose-300 border border-rose-500/50' : 'bg-emerald-500/20 text-emerald-400'}`}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px]">
                      <div>
                        ...
                        <span className={isCorrupted ? 'text-rose-400 font-bold underline' : 'text-slate-400'}>{entry.previousHash}</span>
                      </div>
                      <div>

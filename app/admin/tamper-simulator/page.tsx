...
import { Database, AlertTriangle, ShieldCheck, RefreshCw, CheckCircle2, XCircle, ArrowRight, Layers, ShieldAlert } from 'lucide-react';
      ...
      <main className="flex-1 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full space-y-8">
        ...
        <div className="space-y-2">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs font-semibold">
            ...
            SIH Judge Demonstration Tool — CERTX Admin Portal
          </div>
          <h1 className="text-3xl font-extrabold text-white">Controlled Demo Tamper Simulator</h1>
          <p className="text-xs text-slate-400">
            Demonstrate CERTX's append-only tamper-evident ledger to judges. Deliberately tamper with a demo ledger block to trigger the RED alert, then restore the pristine chain.
          </p>
        </div>
        ...
        <div className="bg-navy-900/80 p-6 rounded-2xl border border-slate-800 space-y-4">
          ...
          <div className="flex flex-wrap gap-4">
            <button
              ...
              disabled={loading}
              className="px-5 py-3 bg-rose-950/80 hover:bg-rose-900 border border-rose-700 text-rose-300 font-bold rounded-xl text-xs transition-all flex items-center gap-2 shadow-lg"
            >
        ...
        {report && (
          <div className={`p-6 rounded-2xl border ${report.isValid ? 'bg-emerald-950/60 border-emerald-500/50' : 'bg-rose-950/80 border-rose-600'}`}>
            <div className="flex items-center justify-between border-b border-slate-800 pb-3 mb-4">
              <div className="flex items-center gap-3">
                {report.isValid ? <CheckCircle2 className="w-6 h-6 text-emerald-400" /> : <XCircle className="w-6 h-6 text-rose-400" />}
            ...
            <div className="space-y-3 font-mono text-xs">
              {report.firstCompromisedIndex && (
                <div className="p-4 rounded-xl bg-rose-900/60 border border-rose-700 text-rose-200 space-y-1">
              ...
              <div className="max-h-80 overflow-y-auto space-y-2 pr-1">
                {report.blocks?.map((b: any) => (
                  <div key={b.id} className={`p-3 rounded-lg border flex items-center justify-between text-[11px] ${b.isValid ? 'bg-navy-950 border-slate-800' : 'bg-rose-900/40 border-rose-600 text-rose-200'}`}>
                    <div>
                    ...
                    <div className="text-right">
                      <span className={`font-bold ${b.isValid ? 'text-emerald-400' : 'text-rose-400'}`}>

    ...
    <div className="space-y-8 py-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/30 text-blue-400 text-xs font-semibold mb-2">
            ...
            National Institute of Technology (NIT Demo) Portal
          </div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-white">Certificate Management</h1>
          <p className="text-xs text-slate-400">Issue cryptographically signed certificates and manage status lifecycles.</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-navy-900/60 p-4 rounded-xl border border-slate-800 space-y-1">
          ...
          <p className="text-xl font-bold text-red-400">{revokedCount}</p>
        </div>
      </div>
          ...
          <table className="w-full text-xs text-left">
            ...
            <tbody className="divide-y divide-slate-800">
              {certificates.map((cert) => {
                ...
                if (cert.status === 'REVOKED') badgeStyle = 'bg-red-500/20 text-red-400 border-red-500/40';
                    ...
                    <td className="p-3.5 text-right space-x-2">
                      ...
                      {cert.status !== 'REVOKED' && (
                        <button
                          ...
                          className="px-2.5 py-1 rounded bg-red-950 text-red-300 hover:bg-red-900 text-[11px] font-semibold"
                        >
                          Revoke
                        </button>
                      )}
